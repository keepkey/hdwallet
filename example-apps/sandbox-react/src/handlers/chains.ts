import * as core from "@keepkey/hdwallet-core";
import * as sigUtil from "@metamask/eth-sig-util";
import { TypedData } from "eip-712";
import Web3 from "web3";
import type { HDWallet } from "@keepkey/hdwallet-core";
import type { SetResult } from "./types";

import {
  arkeoAddClaimTx,
  arkeoBondProviderTx,
  arkeoClaimArkeoTx,
  arkeoClaimContractIncomeTx,
  arkeoClaimEthTx,
  arkeoCloseContractTx,
  arkeoModProviderTx,
  arkeoOpenContractTx,
  arkeoTransferClaimTx,
} from "@/json/arkeo/arkeoAminoTx.json";
import * as bnbTxJson from "@/json/bnbTx.json";
import * as btcBech32TxJson from "@/json/btcBech32Tx.json";
import * as btcSegWitTxJson from "@/json/btcSegWitTx.json";
import * as btcTxJson from "@/json/btcTx.json";
import {
  cosmosDelegateTx,
  cosmosIBCTransferTx,
  cosmosRedelegateTx,
  cosmosRewardsTx,
  cosmosTransferTx,
  cosmosUndelegateTx,
} from "@/json/cosmos/cosmosAminoTx.json";
import * as dashTxJson from "@/json/dashTx.json";
import * as dogeTxJson from "@/json/dogeTx.json";
import { eip712, txs } from "@/json/ethereum/ethTx.json";
import { openSeaListNFTMessage } from "@/json/ethereum/OpenSea-ethSignTypedDataV4.json";
import * as ltcTxJson from "@/json/ltcTx.json";
import { mayachainUnsignedTx } from "@/json/mayachainTx.json";
import {
  osmosisDelegateTx,
  osmosisIBCTransferTx,
  osmosisLPAddTx,
  osmosisLPRemoveTx,
  osmosisRedelegateTx,
  osmosisRewardsTx,
  osmosisSendTx,
  osmosisSwapTx,
  osmosisUndelegateTx,
} from "@/json/osmosis/osmosisAminoTx.json";
import * as rippleTxJson from "@/json/rippleTx.json";
import {
  thorchainBinanceBaseTx,
  thorchainBitcoinBaseTx,
  thorchainEthereumBaseTx,
  thorchainNativeRuneBaseTx,
  thorchainRouterAbi,
  thorchainUnsignedTx,
} from "@/json/thorchainTx.json";

const mnemonic = "alcohol woman abuse must during monitor noble actual mixed trade anger aisle";

function noWallet(setResult: SetResult, section: string, label?: string) {
  setResult(section, label ? `${label} does not support this` : "No wallet?");
}

// --- Eos ---
export async function eosAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsEos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Eos");
    return;
  }
  const { addressNList } = wallet.eosGetAccountPaths({ accountIdx: 0 })[0];
  let result = await wallet.eosGetPublicKey({ addressNList, showDisplay: false });
  result = await wallet.eosGetPublicKey({ addressNList, showDisplay: true, kind: 0 });
  setResult(section, result);
}

export async function eosTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsEos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Eos");
    return;
  }
  const unsigned_main = {
    expiration: "2020-04-30T22:00:00.000",
    ref_block_num: 54661,
    ref_block_prefix: 2118672142,
    max_net_usage_words: 0,
    max_cpu_usage_ms: 0,
    delay_sec: 0,
    context_free_actions: [],
    actions: [
      {
        account: "eosio.token",
        name: "transfer",
        authorization: [{ actor: "xhackmebrosx", permission: "active" }],
        data: { from: "xhackmebrosx", to: "xhighlanderx", quantity: "0.0001 EOS", memo: "testmemo" },
      },
    ],
  };
  const chainid_main = "aca376f206b8fc25a6ed44dbdc66547c36c6c33e3a119ffbeaef943642f0e906";
  const res = await wallet.eosSignTx({
    addressNList: core.bip32ToAddressNList("m/44'/194'/0'/0/0"),
    chain_id: chainid_main,
    tx: unsigned_main,
  });
  setResult(section, res.eosFormSig);
}

// --- Binance ---
export async function binanceAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBinance(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Binance");
    return;
  }
  const { addressNList } = wallet.binanceGetAccountPaths({ accountIdx: 0 })[0];
  let result = await wallet.binanceGetAddress({ addressNList, showDisplay: false });
  result = await wallet.binanceGetAddress({ addressNList, showDisplay: true, address: result });
  setResult(section, result);
}

export async function binanceTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBinance(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Binance");
    return;
  }
  const res = await wallet.binanceSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/714'/0'/0/0`),
    account_number: "24250",
    sequence: 31,
    tx: bnbTxJson,
  });
  setResult(section, JSON.stringify(res));
}

// --- Ripple ---
export async function rippleAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsRipple(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Ripple");
    return;
  }
  const { addressNList } = wallet.rippleGetAccountPaths({ accountIdx: 0 })[0];
  const result = await wallet.rippleGetAddress({ addressNList, showDisplay: true });
  setResult(section, result);
}

export async function rippleTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsRipple(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Ripple");
    return;
  }
  const res = await wallet.rippleSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/144'/0'/0/0`),
    tx: rippleTxJson,
    flags: undefined,
    sequence: "3",
    lastLedgerSequence: "0",
    payment: { amount: "47000", destination: "rEpwmtmvx8gkMhX5NLdU3vutQt7dor4MZm", destinationTag: "1234567890" },
  });
  setResult(section, JSON.stringify(res));
}

// --- Cosmos ---
export async function cosmosAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsCosmos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Cosmos");
    return;
  }
  const { addressNList } = wallet.cosmosGetAccountPaths({ accountIdx: 0 })[0];
  const result = await wallet.cosmosGetAddress({ addressNList, showDisplay: false });
  await wallet.cosmosGetAddress({ addressNList, showDisplay: true });
  setResult(section, result);
}

async function cosmosSignTx(
  wallet: HDWallet,
  setResult: SetResult,
  section: string,
  unsigned: core.Cosmos.StdTx,
  sequence: string
) {
  const res = await wallet.cosmosSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/118'/0'/0/0`),
    chain_id: "cosmoshub-4",
    account_number: "16359",
    sequence,
    tx: unsigned,
  });
  setResult(section, JSON.stringify(res));
}

export async function cosmosTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsCosmos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Cosmos");
    return;
  }
  await cosmosSignTx(wallet, setResult, section, cosmosTransferTx, "17");
}

export async function cosmosDelegate(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsCosmos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Cosmos");
    return;
  }
  await cosmosSignTx(wallet, setResult, section, cosmosDelegateTx, "18");
}

export async function cosmosUndelegate(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsCosmos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Cosmos");
    return;
  }
  await cosmosSignTx(wallet, setResult, section, cosmosUndelegateTx, "20");
}

export async function cosmosRedelegate(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsCosmos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Cosmos");
    return;
  }
  await cosmosSignTx(wallet, setResult, section, cosmosRedelegateTx, "19");
}

export async function cosmosRewards(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsCosmos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Cosmos");
    return;
  }
  await cosmosSignTx(wallet, setResult, section, cosmosRewardsTx, "19");
}

export async function cosmosIBCTransfer(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsCosmos(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Cosmos");
    return;
  }
  await cosmosSignTx(wallet, setResult, section, cosmosIBCTransferTx, "27");
}

// --- Osmosis ---
async function osmosisSignTx(
  wallet: HDWallet,
  setResult: SetResult,
  section: string,
  unsigned: core.Osmosis.StdTx
) {
  const res = await wallet.osmosisSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/118'/0'/0/0`),
    chain_id: unsigned.chain_id,
    account_number: unsigned.account_number,
    sequence: unsigned.sequence,
    tx: unsigned,
  });
  setResult(section, JSON.stringify(res));
}

export async function osmosisAddress(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  const { addressNList } = wallet.osmosisGetAccountPaths({ accountIdx: 0 })[0];
  const result = await wallet.osmosisGetAddress({ addressNList, showDisplay: false });
  await wallet.osmosisGetAddress({ addressNList, showDisplay: true });
  setResult(section, result);
}

export async function osmosisSend(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisSendTx);
}

export async function osmosisDelegate(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisDelegateTx);
}

export async function osmosisUndelegate(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisUndelegateTx);
}

export async function osmosisRedelegate(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisRedelegateTx);
}

export async function osmosisRewards(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisRewardsTx);
}

export async function osmosisLPAdd(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisLPAddTx);
}

export async function osmosisLPRemove(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisLPRemoveTx);
}

export async function osmosisIBCTransfer(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisIBCTransferTx);
}

export async function osmosisSwap(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsOsmosis(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Osmosis");
    return;
  }
  await osmosisSignTx(wallet, setResult, section, osmosisSwapTx);
}

// --- Arkeo ---
async function arkeoSignTx(
  wallet: HDWallet,
  setResult: SetResult,
  section: string,
  unsigned: core.Arkeo.StdTx
) {
  const res = await wallet.arkeoSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/118'/0'/0/0`),
    chain_id: unsigned.chain_id,
    account_number: unsigned.account_number,
    sequence: unsigned.sequence,
    tx: unsigned,
  });
  setResult(section, JSON.stringify(res));
}

export async function arkeoGetAddress(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  const { addressNList } = wallet.arkeoGetAccountPaths({ accountIdx: 0 })[0];
  const result = await wallet.arkeoGetAddress({ addressNList, showDisplay: false });
  await wallet.arkeoGetAddress({ addressNList, showDisplay: true });
  setResult(section, result);
}

export async function arkeoBondProvider(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoBondProviderTx);
}

export async function arkeoModProvider(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoModProviderTx);
}

export async function arkeoOpenContract(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoOpenContractTx);
}

export async function arkeoCloseContract(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoCloseContractTx);
}

export async function arkeoClaimContractIncome(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoClaimContractIncomeTx);
}

export async function arkeoClaimETH(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoClaimEthTx);
}

export async function arkeoClaimArkeo(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoClaimArkeoTx);
}

export async function arkeoTransferClaim(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoTransferClaimTx);
}

export async function arkeoAddClaim(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsArkeo(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Arkeo");
    return;
  }
  await arkeoSignTx(wallet, setResult, section, arkeoAddClaimTx);
}

// --- MAYAChain ---
export async function mayachainAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  const { addressNList } = wallet.mayachainGetAccountPaths({ accountIdx: 0 })[0];
  const result = await wallet.mayachainGetAddress({ addressNList, showDisplay: false });
  await wallet.mayachainGetAddress({ addressNList, showDisplay: true });
  setResult(section, result);
}

export async function mayachainTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  const res = await wallet.mayachainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "mayachain-mainnet-v1",
    account_number: "6359",
    sequence: "15",
    tx: mayachainUnsignedTx,
  });
  setResult(section, JSON.stringify(res));
}

// --- THORChain native ---
export async function thorchainAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const { addressNList } = wallet.thorchainGetAccountPaths({ accountIdx: 0 })[0];
  const result = await wallet.thorchainGetAddress({ addressNList, showDisplay: false });
  await wallet.thorchainGetAddress({ addressNList, showDisplay: true });
  setResult(section, result);
}

export async function thorchainTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: thorchainUnsignedTx,
  });
  setResult(section, JSON.stringify(res));
}

// THORChain Arkeo (via thorchainSignTx)
export async function thorchainArkeoBondProvider(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: arkeoBondProviderTx,
  });
  setResult(section, JSON.stringify(res));
}

export async function thorchainArkeoModProvider(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: arkeoModProviderTx,
  });
  setResult(section, JSON.stringify(res));
}

export async function thorchainArkeoOpenContract(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: arkeoOpenContractTx,
  });
  setResult(section, JSON.stringify(res));
}

export async function thorchainArkeoCloseContract(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: arkeoCloseContractTx,
  });
  setResult(section, JSON.stringify(res));
}

export async function thorchainArkeoClaimContractIncome(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: arkeoClaimContractIncomeTx,
  });
  setResult(section, JSON.stringify(res));
}

export async function thorchainArkeoClaimEth(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: { ...arkeoClaimEthTx, signatures: [] },
  });
  setResult(section, JSON.stringify(res));
}

export async function thorchainArkeoClaimArkeo(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: { ...arkeoClaimArkeoTx, signatures: [] },
  });
  setResult(section, JSON.stringify(res));
}

export async function thorchainArkeoTransferClaim(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: { ...arkeoTransferClaimTx, signatures: [] },
  });
  setResult(section, JSON.stringify(res));
}

export async function thorchainArkeoAddClaim(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsThorchain(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support THORChain");
    return;
  }
  const res = await wallet.thorchainSignTx({
    addressNList: core.bip32ToAddressNList(`m/44'/931'/0'/0/0`),
    chain_id: "thorchain",
    account_number: "24250",
    sequence: "3",
    tx: { ...arkeoAddClaimTx, signatures: [] },
  });
  setResult(section, JSON.stringify(res));
}

// --- Ethereum ---
const doesRecoveredMatchAddress = (typedData: TypedData, fromAddress: string, signature: string): boolean => {
  const recoveredAddress = sigUtil.recoverTypedSignature({
    data: typedData as Parameters<typeof sigUtil.recoverTypedSignature>[0]["data"],
    signature,
    version: sigUtil.SignTypedDataVersion.V4,
  });
  return Web3.utils.toChecksumAddress(recoveredAddress) === Web3.utils.toChecksumAddress(fromAddress);
};

export async function ethAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsETH(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support ETH");
    return;
  }
  const { hardenedPath, relPath } = (wallet as unknown as core.ETHWalletInfo).ethGetAccountPaths({ coin: "Ethereum", accountIdx: 0 })[0];
  let result = await wallet.ethGetAddress({ addressNList: hardenedPath.concat(relPath), showDisplay: false });
  result = await wallet.ethGetAddress({ addressNList: hardenedPath.concat(relPath), showDisplay: true });
  setResult(section, result ?? "");
}

export async function ethTx(
  wallet: HDWallet | null,
  setResult: SetResult,
  section: string,
  useEIP1559: boolean
) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsETH(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support ETH");
    return;
  }
  const tx = useEIP1559 ? txs.ethTx1559 : txs.ethTxLegacy;
  const res = await wallet.ethSignTx(tx);
  setResult(section, JSON.stringify(res));
}

export async function ethSign(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsETH(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support ETH");
    return;
  }
  const { hardenedPath, relPath } = (wallet as unknown as core.ETHWalletInfo).ethGetAccountPaths({ coin: "Ethereum", accountIdx: 0 })[0];
  const result = await wallet.ethSignMessage({
    addressNList: hardenedPath.concat(relPath),
    message: "0x48656c6c6f20576f726c64",
  });
  setResult(section, result != null ? `Address: ${result.address} Signature: ${result.signature}` : "Failed to sign");
}

export async function ethSend(
  wallet: HDWallet | null,
  setResult: SetResult,
  section: string,
  useEIP1559: boolean
) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsETH(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support ETH");
    return;
  }
  const tx = useEIP1559 ? txs.ethTx1559 : txs.ethTxLegacy;
  if (wallet.ethSendTx) {
    const result = await wallet.ethSendTx(tx as core.ETHSignTx);
    setResult(section, result?.hash ?? "Failed to send");
  } else {
    const label = await wallet.getVendor();
    setResult(section, label + " does not support ethSendTx");
  }
}

export async function ethVerify(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsETH(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support ETH");
    return;
  }
  const signerAddress = "0x2068dD92B6690255553141Dfcf00dF308281f763";
  const result = await wallet.ethVerifyMessage({
    address: signerAddress,
    message: "Hello World",
    signature:
      "0x61f1dda82e9c3800e960894396c9ce8164fd1526fccb136c71b88442405f7d09721725629915d10bc7cecfca2818fe76bc5816ed96a1b0cebee9b03b052980131b",
  });
  setResult(section, result ? `✅ ${signerAddress} is the signer` : `❌ ${signerAddress} is NOT the signer`);
}

export async function ethSignTypedData(
  wallet: HDWallet | null,
  setResult: SetResult,
  section: string,
  typedData: TypedData
) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsETH(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support ETH");
    return;
  }
  const { hardenedPath, relPath } = (wallet as unknown as core.ETHWalletInfo).ethGetAccountPaths({ coin: "Ethereum", accountIdx: 0 })[0];
  const signed = wallet.ethSignTypedData
    ? await wallet.ethSignTypedData({ typedData, addressNList: hardenedPath.concat(relPath) })
    : null;
  if (!signed) {
    setResult(section, "Wallet does not support ethSignTypedData or signing failed");
    return;
  }
  const { address, signature } = signed;
  const success = doesRecoveredMatchAddress(typedData, address, signature);
  setResult(
    section,
    (success ? `✅ "${address}" is the signer` : `❌ "${address}" is NOT the signer`) + `; Signature is: "${signature}"`
  );
}

// --- Bitcoin ---
export async function btcAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BTC");
    return;
  }
  const res = await wallet.btcGetAddress({
    addressNList: [0x80000000 + 44, 0x80000000 + 0, 0x80000000 + 0, 0, 0],
    coin: "Bitcoin",
    scriptType: core.BTCInputScriptType.SpendAddress,
    showDisplay: true,
  });
  setResult(section, res ?? "");
}

export async function btcTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BTC");
    return;
  }
  const txid = "b3002cd9c033f4f3c2ee5a374673d7698b13c7f3525c1ae49a00d2e28e8678ea";
  const hex =
    "";
  const inputs = [
    {
      addressNList: [0x80000000 + 44, 0x80000000 + 0, 0x80000000 + 0, 0, 0],
      scriptType: core.BTCInputScriptType.SpendAddress,
      amount: String(10000),
      vout: 1,
      txid,
      tx: btcTxJson,
      hex,
    },
  ];
  const outputs = [
    {
      address: "1MJ2tj2ThBE62zXbBYA5ZaN3fdve5CPAz1",
      addressType: core.BTCOutputAddressType.Spend,
      scriptType: core.BTCOutputScriptType.PayToAddress,
      amount: String(10000 - 1000),
      isChange: false,
    },
  ];
  const res = await wallet.btcSignTx({
    coin: "Bitcoin",
    inputs: inputs as unknown as core.BTCSignTxInput[],
    outputs: outputs as unknown as core.BTCSignTxOutput[],
    version: 1,
    locktime: 0,
  });
  setResult(section, res?.serializedTx ?? "Failed to sign");
}

export async function btcSign(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BTC");
    return;
  }
  const res = await wallet.btcSignMessage({
    addressNList: core.bip32ToAddressNList("m/44'/0'/0'/0/0"),
    coin: "Bitcoin",
    scriptType: core.BTCInputScriptType.SpendAddress,
    message: "Hello World",
  });
  setResult(section, res != null ? res.address + " " + res.signature : "Failed to sign");
}

export async function btcVerify(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BTC");
    return;
  }
  const res = await wallet.btcVerifyMessage({
    address: "1FH6ehAd5ZFXCM1cLGzHxK1s4dGdq1JusM",
    coin: "Bitcoin",
    signature:
      "20a037c911044cd6c851b6508317d8892067b0b62074b2cf1c0df9abd4aa053a3c243ffdc37f64d7af2c857128eafc81947c380995596615e5dcc313a15f512cdd",
    message: "Hello World",
  });
  setResult(section, res ? "✅" : "❌");
}

// Bitcoin Segwit
export async function btcAddrSegWit(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BTC");
    return;
  }
  const res = await wallet.btcGetAddress({
    addressNList: [0x80000000 + 49, 0x80000000 + 0, 0x80000000 + 0, 0, 0],
    coin: "Bitcoin",
    scriptType: core.BTCInputScriptType.SpendP2SHWitness,
    showDisplay: true,
  });
  setResult(section, res ?? "");
}

export async function btcAddrSegWitNative(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BTC");
    return;
  }
  const res = await wallet.btcGetAddress({
    addressNList: [0x80000000 + 84, 0x80000000 + 0, 0x80000000 + 0, 0, 0],
    coin: "Bitcoin",
    scriptType: core.BTCInputScriptType.SpendWitness,
    showDisplay: true,
  });
  setResult(section, res ?? "");
}

export async function btcTxSegWit(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BTC");
    return;
  }
  const txid = "609410a9eac51cdce2b9c1911c7b8705bc566e164bca07ae25f2dee87b5b6a91";
  const hex =
    "";
  const inputs = [
    {
      addressNList: [0x80000000 + 49, 0x80000000 + 0, 0x80000000 + 0, 0, 0],
      amount: String(100000),
      vout: 0,
      txid,
      scriptType: core.BTCInputScriptType.SpendP2SHWitness,
      tx: btcSegWitTxJson,
      hex,
    },
  ];
  const outputs = [
    {
      address: "3Eq3agTHEhMCC8sZHnJJcCcZFB7BBSJKWr",
      addressType: core.BTCOutputAddressType.Spend,
      scriptType: core.BTCOutputScriptType.PayToAddress,
      amount: String(89869),
      isChange: false,
    },
  ];
  const res = await wallet.btcSignTx({
    coin: "Bitcoin",
    inputs: inputs as unknown as core.BTCSignTxInput[],
    outputs: outputs as unknown as core.BTCSignTxOutput[],
    version: 1,
    locktime: 0,
  });
  setResult(section, res?.serializedTx ?? "Failed to sign");
}

export async function btcTxSegWitNative(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  await wallet.loadDevice({ mnemonic: "all all all all all all all all all all all all" });
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BTC");
    await wallet.loadDevice({ mnemonic });
    return;
  }
  const txid = "fa80a9949f1094119195064462f54d0e0eabd3139becd4514ae635b8c7fe3a46";
  const hex =
    "";
  const inputs = [
    {
      addressNList: [0x80000000 + 84, 0x80000000 + 0, 0x80000000 + 0, 0, 0],
      amount: String(9426),
      vout: 0,
      txid,
      scriptType: core.BTCInputScriptType.SpendWitness,
      tx: btcBech32TxJson,
      hex,
    },
  ];
  const outputs = [
    {
      address: "bc1qc5dgazasye0yrzdavnw6wau5up8td8gdqh7t6m",
      addressType: core.BTCOutputAddressType.Spend,
      scriptType: core.BTCOutputScriptType.PayToAddress,
      amount: String(1337),
      isChange: false,
    },
  ];
  const res = await wallet.btcSignTx({
    coin: "Bitcoin",
    inputs: inputs as unknown as core.BTCSignTxInput[],
    outputs: outputs as unknown as core.BTCSignTxOutput[],
    version: 1,
    locktime: 0,
  });
  setResult(section, res?.serializedTx ?? "Failed to sign");
  await wallet.loadDevice({ mnemonic });
}

// --- Litecoin ---
const ltcBip44 = {
  scriptType: core.BTCInputScriptType.SpendAddress,
  addressNList: [0x80000000 + 44, 0x80000000 + 2, 0x80000000 + 0, 0, 0],
};

export async function ltcAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Litecoin");
    return;
  }
  const res = await wallet.btcGetAddress({
    addressNList: ltcBip44.addressNList,
    coin: "Litecoin",
    scriptType: ltcBip44.scriptType,
    showDisplay: true,
  });
  setResult(section, res ?? "");
}

export async function ltcTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Litecoin");
    return;
  }
  const txid = "1de79c706f34c81bbefad49a9ff8d12b6ca86b77605a1998505e4f8792a5892d";
  const hex =
    "";
  const inputs = [
    {
      addressNList: ltcBip44.addressNList,
      scriptType: core.BTCInputScriptType.SpendAddress,
      amount: String(2160258),
      vout: 0,
      txid,
      segwit: false,
      tx: ltcTxJson,
      hex,
    },
  ];
  const outputs = [
    {
      address: "LLe4PciAJgMMJSAtQQ5nkC13t6SSMmERJ3",
      addressType: core.BTCOutputAddressType.Spend,
      scriptType: core.BTCOutputScriptType.PayToAddress,
      amount: String(261614),
      isChange: false,
    },
  ];
  const res = await wallet.btcSignTx({
    coin: "Litecoin",
    inputs: inputs as unknown as core.BTCSignTxInput[],
    outputs: outputs as unknown as core.BTCSignTxOutput[],
    version: 1,
    locktime: 0,
  });
  setResult(section, res?.serializedTx ?? "Failed to sign");
}

export async function ltcSign(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Litecoin");
    return;
  }
  const res = await wallet.btcSignMessage({
    addressNList: ltcBip44.addressNList,
    coin: "Litecoin",
    scriptType: core.BTCInputScriptType.SpendAddress,
    message: "Hello World",
  });
  setResult(section, res != null ? res.address + " " + res.signature : "Failed to sign");
}

// --- Dogecoin ---
const dogeBip44 = {
  scriptType: core.BTCInputScriptType.SpendAddress,
  addressNList: [0x80000000 + 44, 0x80000000 + 3, 0x80000000 + 0],
};

export async function dogeAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support DOGE");
    return;
  }
  const res = await wallet.btcGetAddress({
    addressNList: dogeBip44.addressNList.concat([0, 0]),
    coin: "Dogecoin",
    scriptType: dogeBip44.scriptType,
    showDisplay: true,
  });
  setResult(section, res ?? "");
}

export async function dogeTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support DOGE");
    return;
  }
  const txid = "4ab8c81585bf61ddcba03f4b2f4958b3800d68b02874f4955e258775cb3e7068";
  const hex =
    "";
  const inputs = [
    {
      addressNList: dogeBip44.addressNList.concat([0, 0]),
      scriptType: core.BTCInputScriptType.SpendAddress,
      amount: String(35577380098),
      vout: 1,
      txid,
      segwit: false,
      tx: dogeTxJson,
      hex,
    },
  ];
  const outputs = [
    {
      address: "DMEHVGRsELY5zyYbfgta3pAhedKGeaDeJd",
      addressType: core.BTCOutputAddressType.Spend,
      scriptType: core.BTCOutputScriptType.PayToAddress,
      amount: String(35577380098),
      isChange: false,
    },
  ];
  const res = await wallet.btcSignTx({
    coin: "Dogecoin",
    inputs: inputs as unknown as core.BTCSignTxInput[],
    outputs: outputs as unknown as core.BTCSignTxOutput[],
    version: 1,
    locktime: 0,
  });
  setResult(section, res?.serializedTx ?? "Failed to sign");
}

// --- Bitcoin Cash ---
const bchBip44 = {
  scriptType: core.BTCInputScriptType.SpendAddress,
  addressNList: [0x80000000 + 44, 0x80000000 + 145, 0x80000000 + 0],
};

export async function bchAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BCH");
    return;
  }
  const res = await wallet.btcGetAddress({
    addressNList: bchBip44.addressNList.concat([0, 0]),
    coin: "BitcoinCash",
    scriptType: bchBip44.scriptType,
    showDisplay: true,
  });
  setResult(section, res ?? "");
}

export async function bchTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support BCH");
    return;
  }
  const txid = "35ec5b47eea3b45efb062c6fabad43987a79b855dc42630b34f8d26d4a646a2e";
  const hex =
    "";
  const inputs = [
    {
      addressNList: bchBip44.addressNList.concat([0, 0]),
      scriptType: core.BTCInputScriptType.SpendAddress,
      amount: String(13813640),
      vout: 0,
      txid,
      segwit: false,
      hex,
    },
  ];
  const supportsCashAddr = await wallet.btcSupportsScriptType("BitcoinCash", core.BTCInputScriptType.CashAddr);
  const outputs = [
    {
      address: supportsCashAddr ? "bitcoincash:qq5mg2xtp9y5pvvgy7m4k2af5a7s5suulueyywgvnf" : "14oWXZFPhgP9DA3ggPzhHpUUaikDSjAuMC",
      addressType: core.BTCOutputAddressType.Spend,
      scriptType: core.BTCOutputScriptType.PayToAddress,
      amount: String(13813640),
      isChange: false,
    },
  ];
  const res = await wallet.btcSignTx({
    coin: "BitcoinCash",
    inputs: inputs as unknown as core.BTCSignTxInput[],
    outputs: outputs as unknown as core.BTCSignTxOutput[],
    version: 1,
    locktime: 0,
  });
  setResult(section, res?.serializedTx ?? "Failed to sign");
}

// --- Dash ---
const dashBip44 = {
  scriptType: core.BTCInputScriptType.SpendAddress,
  addressNList: [0x80000000 + 44, 0x80000000 + 5, 0x80000000 + 0],
};

export async function dashAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Dash");
    return;
  }
  const res = await wallet.btcGetAddress({
    addressNList: dashBip44.addressNList.concat([0, 0]),
    coin: "Dash",
    scriptType: dashBip44.scriptType,
    showDisplay: true,
  });
  setResult(section, res ?? "");
}

export async function dashTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support Dash");
    return;
  }
  const txid = "94b3bbe89e5106d93d07be311a543958fa0de127d4bf747e9102c43e92cbb55f";
  const hex =
    "";
  const inputs = [
    {
      addressNList: dashBip44.addressNList.concat([0, 0]),
      scriptType: core.BTCInputScriptType.SpendAddress,
      amount: String(4954359),
      vout: 0,
      txid,
      segwit: false,
      tx: dashTxJson,
      hex,
    },
  ];
  const outputs = [
    {
      address: "XexybzTUtH9V9eY4UJN2aCcBT3utan5C8N",
      addressType: core.BTCOutputAddressType.Spend,
      scriptType: core.BTCOutputScriptType.PayToAddress,
      amount: String(4000000),
      isChange: false,
    },
  ];
  const res = await wallet.btcSignTx({
    coin: "Dash",
    inputs: inputs as unknown as core.BTCSignTxInput[],
    outputs: outputs as unknown as core.BTCSignTxOutput[],
    version: 1,
    locktime: 0,
  });
  setResult(section, res?.serializedTx ?? "Failed to sign");
}

// --- DigiByte ---
const dgbBip44 = {
  scriptType: core.BTCInputScriptType.SpendAddress,
  addressNList: [0x80000000 + 44, 0x80000000 + 20, 0x80000000 + 0],
};

export async function dgbAddr(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support DigiByte");
    return;
  }
  const res = await wallet.btcGetAddress({
    addressNList: dgbBip44.addressNList.concat([0, 0]),
    coin: "DigiByte",
    scriptType: dgbBip44.scriptType,
    showDisplay: true,
  });
  setResult(section, res ?? "");
}

export async function dgbTx(wallet: HDWallet | null, setResult: SetResult, section: string) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  await wallet.loadDevice({ mnemonic: "all all all all all all all all all all all all" });
  if (!core.supportsBTC(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support DigiByte");
    await wallet.loadDevice({ mnemonic });
    return;
  }
  const inputs = [
    {
      addressNList: dgbBip44.addressNList.concat([0, 0]),
      scriptType: core.BTCInputScriptType.SpendAddress,
      amount: String(480000000),
      vout: 15,
      txid: "be150359df4123b379f1f12de978bfced92644645da17b97c7613879f4306a90",
      tx: null,
      hex: "",
    },
    {
      addressNList: dgbBip44.addressNList.concat([0, 0]),
      scriptType: core.BTCInputScriptType.SpendAddress,
      amount: String(10000000),
      vout: 0,
      txid: "528ec23eaf123282e9bce297ebb3edfb05e8b4d5875cbc9c271a98d72a202340",
      tx: null,
      hex: "",
    },
  ];
  const outputs = [
    {
      address: "SWpe93hQL2pLUDLy7swsDPWQJGCHSsgmun",
      addressType: null,
      scriptType: core.BTCOutputScriptType.PayToMultisig,
      amount: String(400000000),
      isChange: false,
    },
    {
      address: "DNLcBry65dHehGExGYjBkM8kxDYr7mZ3BT",
      addressType: null,
      scriptType: core.BTCOutputScriptType.PayToAddress,
      relpath: "1/9",
      amount: String(90000000),
      isChange: true,
      index: 9,
    },
  ];
  const res = await wallet.btcSignTx({
    coin: "DigiByte",
    inputs: inputs as unknown as core.BTCSignTxInput[],
    outputs: outputs as unknown as core.BTCSignTxOutput[],
    version: 1,
    locktime: 0,
  });
  setResult(section, res?.serializedTx ?? "Failed to sign");
  await wallet.loadDevice({ mnemonic });
}

// --- ERC-20 ---
export type Erc20Action =
  | "Address"
  | "Allowance"
  | "Approve"
  | "Balance Of"
  | "Total Supply"
  | "Transfer"
  | "Transfer From";

export async function erc20Submit(
  wallet: HDWallet | null,
  setResult: SetResult,
  section: string,
  action: Erc20Action,
  form: Record<string, string>
) {
  if (!wallet) {
    setResult(section, "No wallet?");
    return;
  }
  if (!core.supportsETH(wallet)) {
    const label = await wallet.getLabel();
    setResult(section, label + " does not support ETH");
    return;
  }
  const { hardenedPath, relPath } = (wallet as unknown as core.ETHWalletInfo).ethGetAccountPaths({ coin: "Ethereum", accountIdx: 0 })[0];
  let result: unknown;
  let data: string | undefined;
  const pad = (s: string, len = 64) => s.replace("0x", "").padStart(len, "0");

  switch (action) {
    case "Address":
      result = await wallet.ethGetAddress({ addressNList: hardenedPath.concat(relPath), showDisplay: false });
      result = await wallet.ethGetAddress({
        addressNList: hardenedPath.concat(relPath),
        showDisplay: true,
      });
      break;
    case "Allowance":
      data =
        "0xdd62ed3e" +
        pad(form.ownerAddress ?? "") +
        pad(form.spenderAddress ?? "");
      result = await wallet.ethSignTx({
        addressNList: hardenedPath.concat(relPath),
        nonce: "0x0",
        gasPrice: "0x5FB9ACA00",
        gasLimit: "0x186A0",
        value: "0x00",
        to: form.contractAddress,
        chainId: 1,
        data,
      });
      break;
    case "Approve":
      data =
        "0x095ea7b3" +
        pad(form.spenderAddress ?? "") +
        parseInt(form.amount ?? "0", 10).toString(16).padStart(64, "0");
      result = await wallet.ethSignTx({
        addressNList: hardenedPath.concat(relPath),
        nonce: "0x0",
        gasPrice: "0x5FB9ACA00",
        gasLimit: "0x186A0",
        value: "0x00",
        to: form.contractAddress,
        chainId: 1,
        data,
      });
      break;
    case "Balance Of":
      data = "0x70a08231" + pad(form.accountAddress ?? "");
      result = await wallet.ethSignTx({
        addressNList: hardenedPath.concat(relPath),
        nonce: "0x0",
        gasPrice: "0x5FB9ACA00",
        gasLimit: "0x186A0",
        value: "0x00",
        to: form.contractAddress,
        chainId: 1,
        data,
      });
      break;
    case "Total Supply":
      data = "0x18160ddd";
      result = await wallet.ethSignTx({
        addressNList: hardenedPath.concat(relPath),
        nonce: "0x0",
        gasPrice: "0x5FB9ACA00",
        gasLimit: "0x186A0",
        value: "0x00",
        to: form.contractAddress,
        chainId: 1,
        data,
      });
      break;
    case "Transfer":
      data =
        "0xa9059cbb" +
        pad(form.recipientAddress ?? "") +
        parseInt(form.amount ?? "0", 10).toString(16).padStart(64, "0");
      result = await wallet.ethSignTx({
        addressNList: hardenedPath.concat(relPath),
        nonce: "0x0",
        gasPrice: "0x5FB9ACA00",
        gasLimit: "0x186A0",
        value: "0x00",
        to: form.contractAddress,
        chainId: 1,
        data,
      });
      break;
    case "Transfer From":
      data =
        "0x23b872dd" +
        pad(form.senderAddress ?? "") +
        pad(form.recipientAddress ?? "") +
        parseInt(form.amount ?? "0", 10).toString(16).padStart(64, "0");
      result = await wallet.ethSignTx({
        addressNList: hardenedPath.concat(relPath),
        nonce: "0x0",
        gasPrice: "0x5FB9ACA00",
        gasLimit: "0x186A0",
        value: "0x00",
        to: form.contractAddress,
        chainId: 1,
        data,
      });
      break;
    default:
      setResult(section, "Unknown action");
      return;
  }
  setResult(section, JSON.stringify(result, null, 4));
}

// Re-export openSea and eip712 for eth sign typed data
export { openSeaListNFTMessage, eip712 };
