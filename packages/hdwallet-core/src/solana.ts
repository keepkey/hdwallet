...
export interface SolanaSignTx {
  ...
  /** @deprecated Host approval only; ignored by the SDK. Opaque transactions
   * require the device's AdvancedMode policy. No wire override exists. */
   ...
   * `lutProof`/`schema`. Required for the certified path — both signerKeyId
   * fields above MUST be 0x80 when this is present, or firmware rejects the
   * request as partial/mixed.
   */
